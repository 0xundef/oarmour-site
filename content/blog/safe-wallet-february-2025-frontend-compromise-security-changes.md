---
title: "After the February 2025 Safe{Wallet} Frontend Compromise: What Changed in Signing, Integrity Checks, and Pre-Sign Review"
...
## Phase 1: Emergency release v1.51.0 (24 February 2025)

Version **v1.51.0** was a focused security release. Each change maps directly to lessons from the incident.
...
### 5. Full raw calldata in advanced transaction details (`#5102`)
...
### 5b. Independent hash verification — Safe Utils (`#5147`)
...
**What we shipped:** A **temporary removal** of Ledger and Trezor modules in v1.51.0.

**Status today:** ❌ **Reversed.** Ledger support returned in **April 2025** (`#5377`); Trezor was re-enabled in **March 2026** (`#7077`), with expanded test coverage and hash-comparison UX for Ledger signing.

---

## Phase 2: Immediate follow-ups (late February – March 2025)

The v1.51.0 release was the floor, not the ceiling.

| Improvement | Intent | Status |
|---|---|---|
| **Blockaid: pass dApp origin** (`#5114`) | Give simulation/scoring engines accurate caller context | ✅ Active |
| **Blockaid: `non_dapp: true` for non-dApp flows** (`#5129`) | Reduce false negatives when origin metadata is absent | ✅ Active |
| **Untrusted fallback handler warnings** (`#4877`) | Flag proxy/fallback contracts that are not on the known-safe list | ✅ Evolved into Safe Shield + Security Hub scanners |
| **Third-party hash validation (Safe Utils)** (`#5147`) | Let signers independently recompute hashes outside the app | ✅ Active — linked from the confirmation receipt |
| **Validated Safe address from URL** (`#5168`) | Prevent loading arbitrary addresses from tampered deep links | ✅ Active via `useSafeAddressFromUrl` |
| **CSP tightening** (`#5201`) | Reduce XSS blast radius | ⚠️ Partially rolled back for analytics (`#5339` reintroduced `unsafe-inline` for GA) |
| **Subresource Integrity (SRI) for dynamic chunks** (`#5199`, `#7026`) | Detect tampered JavaScript bundles at load time | ✅ Active — webpack SRI manifest plugin |
| **Blockaid simulation on confirmation** (`#5203`) | Surface simulation results before sign/execute | ✅ Evolved into Safe Shield widget |
| **Production-only security banner** (`#5170`) | Surface signing-risk messaging on production | ❌ Removed — see below |
| **Security banner removed** (`#5384`, Mar 2025) | Retired global banner in favor of in-flow review | Superseded by Safe Shield / confirmation UX |
...
**What we shipped:** SRI hashes for dynamically loaded webpack chunks (`#5199`, later reinforced in `#7026`), so the browser refuses to execute bundles that do not match the expected digest.
...
**What we shipped:** Tighter `script-src` in `#5201`.

**Status today:** ⚠️ **Partially rolled back** — `#5339` reintroduced `'unsafe-inline'` for Google Analytics. CSP still constrains many vectors, but inline script policy is a known trade-off.

### Untrusted fallback handler — single transactions and MultiSend

| Change | PR | Intent |
|---|---|---|
| First-line warnings for unknown fallback handlers | `#4877` | Alert when proxy/fallback is not on the known-safe list |
| Same checks inside **MultiSend** batches | `#5756` | Close the batching loophole—attackers often hide malicious calls inside aggregated transactions |
| Pre-sign check in Safe Shield | `#6902` | Flag **unofficial** fallback handlers during transaction review |
...
- Restored **master copy warnings** on the dashboard Action Required panel (`#7250`, after a brief revert in `#7249`).
- **Security Hub** scanners for factory validation, implementation version, and L2 master copy staleness (`#7839`, `#7898`).

**Status today:** ✅ **Active** — dashboard warnings and Hub scans complement each other.

### Production security messaging (retired)

After v1.51.0, we briefly showed a **production-only security banner** (`#5170`) to reinforce signing hygiene. In **March 2025** it was **removed** (`#5384`) in favor of in-context review (Safe Utils links, expandable calldata, later Safe Shield)—avoiding alert fatigue while keeping controls at the point of signing.
...
- **Mandatory risk acknowledgment** for critical-severity findings — evolved from the earlier "disable Continue until Blockaid warning confirmed" pattern (`#5744`)
- **Off-chain message signing** coverage (`#6748`)
- **EIP-712 typed message analysis** (`#7409`)
- **Nested `approveHash` transaction threat merging** (`#6858`, after a brief revert in `#6855` and re-merge)
...
|---|---|---|
| Pass correct **dApp origin** to Blockaid | `#5114`, `#7663` | Simulation must reflect who invoked the transaction |
| Parse **Safe App origin JSON** before sending to Blockaid | `#6400` | Prevents malformed or spoofed origin metadata from skewing results |
| **`non_dapp: true`** when no dApp context exists | `#5129` | Avoids silent simulation skips on non–Safe App flows |
| **Temporarily disable Guard** during Blockaid simulation | `#6501`, `#5956` | Stops Guard from masking simulation output; simulation uses zero-address guard override where needed |
...
One deliberate omission: **signer screening via Blockaid** was temporarily removed (`#7895`) because the backend endpoint was not yet production-ready—shipping a "Clear" grade while individual signers showed "Screening unavailable" was misleading. The checks matrix documents this as **planned**, not forgotten.

**Post-launch Hub hardening** (still incident-aligned—accurate posture signals prevent false confidence after a trust shock):

| Fix | PR | Intent |
|---|---|---|
| Strict majority rule in account-setup scoring | `#7894` | Correct threshold/signers grading |
| Consistent **$1M balance** threshold for guard recommendations | `#7893` | Align advice with documented policy |
| Stable score + hardened re-scan | `#7933` | Prevent flickering grades between scans |
| Gauge no longer flips during scan | `#7874` | UX trust in the score itself |
...
- **Spaces login gating** (`#7884`) ensures unauthenticated users cannot reach wallet operations without an explicit workspace context.
...
- **Artifact attestation** in CI (`#5133`)
...
| **24 Feb 2025** | **v1.51.0** emergency security release |
| **25 Feb – Mar 2025** | Blockaid origin fixes, Safe Utils link, CSP/SRI, validated address parsing, security banner retired |
| **May 2025** | MultiSend untrusted-fallback warnings (`#5756`) |
| **Late 2025** | Safe Shield widget; Blockaid must-confirm → risk acknowledgment (`#5744`) |
| **Jan–Mar 2026** | SRI reinforcement (`#7026`), EIP-712 Shield analysis (`#7409`), Trezor re-enabled (`#7077`) |
| **May 2026** | Security Hub launch (`#7839`) + scoring fixes (`#7893`–`#7895`, `#7933`, `#7874`) |
